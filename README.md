...
Sprint 5.8 stärker [förhandskontrollen av modulpaket](./MODULE_GATE_PROFILE.md): 54 frysta fall kontrollerar digest, låsning, grants och funktionskontrakt. Dubbla funktionsnamn och feltypade deklarationer stoppas före modulens startkod. Rapporten skiljer detta från exportfel efter laddning och kan reproduceras med `conformance-module-gate`. Grinden använder den faktiska JavaScript-kärnan och gör inget anspråk på oberoende modulexekvering eller sandbox-säkerhet.

Sprint 5.9 tillför [oberoende paketkontroll](./MODULE_ADMISSION_PROFILE.md): 72 frysta fall jämför JavaScript-kärnans förhandsbeslut med en fristående Python-implementation som verifierar manifest, låsning, UTF-8-digests och grants utan att köra modulernas källkod. Rapporten skiljer godkänd förhandskontroll från senare export- och körningsfel. Ett ogiltigt senare paket stoppar även det första paketets startkod. `conformance-module-admission` reproducerar jämförelsen; oberoendet gäller paketkontroll och innebär inte oberoende modulexekvering.
