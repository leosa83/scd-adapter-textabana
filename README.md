...
Fem interaktiva labs visar olika projektioner av samma worker-run:
...
Fixturepaketet innehåller `scope-torture`, `editor-revision`, `channel-fanout`, `base64-inverse`, `failed-run`, `data-join` och `notebook-snapshot`.
...
`org.textabana.result-summary` är en körbar, deterministisk referensadapter. `org.textabana.data-table` är en körbar `data/1` playground-subset. `org.textabana.notebook` producerar en host-neutral, source-bound JSON-projektion från fyra kanoniska notebookkanaler. Endast annotationsadaptern återstår som `contract-only`.
...
Dokumentationen är **Textabana Language & Interop draft 0.5**. Webbmotorn implementerar uttryckligen avgränsade playground-subsets av `language-core/0.4`, `runtime-json/1`, `editor/1`, `adapter-contract/1`, `data/1` och `notebook/1`; den gör ännu inte anspråk på full profilkonformitet. UI:t redovisar funktioner som ännu saknas som `contract-only`, `unsupported` eller `planned`.
...
- `app/playground-labs.tsx` — de fem resultatprojektionerna.
