...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "syntax", label: "Parser, syntax & recovery" },
  ...
  },
  {
    ...
    items: [
      { id: "ir-plan", label: "IR, graf, plan & körspår" },
...
function SectionHeading({
  ...
  const implementationLabel = {
    ...
    defined: "Definierat i draft 0.7",
    ...
    <div className="docs-layout spec-layout">
      ...
      <aside className="docs-index spec-index">
        ...
        <div className="spec-legend" aria-label="Statusförklaring">
          ...
          <span><i className="defined" /> Normativt definierat i draft 0.7</span>
      ...
      <main className="docs-content spec-content" id="spec-main" tabIndex={-1}>
        <details className="mobile-spec-index">
          <summary>Innehåll · Interop 0.7</summary>
          <SpecNav mobile />
        </details>
        ...
        <section className="spec-hero" id="definition" aria-labelledby="definition-title">
          ...
          <div className="definition-grid">
            ...
            <article><Braces aria-hidden="true" /><strong>Explicit parserkedja</strong><p>Lossless CST blir AST, typed IR och därefter en deterministisk körplan — eller lokal, icke-körbar recovery.</p></article>
        ...
        <section className="docs-section spec-section" id="status">
          ...
          <SpecTable
            ...
            rows={[
              ...
              [<code key="i">Implementation</code>, "Körbar language 0.4-subset · Interaktiv subset · Definierat i draft 0.7 · Contract-only", "Anger vad webbplaygrounden faktiskt kör, vad den endast visualiserar delvis och vad som fortfarande är ett kontrakt för kommande implementation."],
            ]}
          />
        ...
        <section className="docs-section spec-section" id="architecture">
          ...
          <div className="architecture-flow" aria-label="Textabanas arkitektur">
            ...
            <ArrowDown aria-hidden="true" />
            <div><Braces aria-hidden="true" /><strong>Parser + compiler</strong><span>CST · AST · IR · Plan/DAG</span></div>
            <ArrowDown aria-hidden="true" />
        ...
        <section className="docs-section spec-section contract-section" id="contract">
          ...
          <ol className="invariant-list">
            ...
            <li><span>17</span><p>Hela source snapshot parsas före modulinitiering. Recovery är synlig för editorn men aldrig körbar.</p></li>
          </ol>
        </section>
        ...
        <section className="docs-section spec-section" id="syntax">
          <SectionHeading number="05" layer="Language" title="Formell grammatik, argument och lokal recovery" implementation="partial" />
          <p className="lead">Modaliteten är synlig direkt i texten. En versionssatt Lezer-parser producerar först en förlustfri CST; Textabana sänker den därefter till AST och typed IR. Samma parseprodukt styr include-resolution, inspektion och exekvering.</p>
          <SpecTable
            ...
            rows={[
              ...
              [<code key="escape">\&gt;&gt;&gt;&gt;</code>, "Escapad markör", "Blir literal text; exakt escape-backslash tas bort."],
              [<code key="fence">``` / ~~~</code>, "Literal fence", "Kontrollmarkörer inuti Markdown-fence tolkas aldrig."],
            ]}
          />
          <CodeExample
          ...
          />
          <CodeExample
            title="Körbar EBNF · förenklat kärnfragment"
            language="ebnf"
            status="Parser lab-v1"
            code={code(
              "Document      ::= Item*",
          ...
          <div className="rule-grid compact">
            <article><strong>Funktionsargument</strong><p><code>sentences=2</code> och andra vanliga argument typas av parsern och skickas till funktionen. Validering mot funktionens fulla JSON Schema är definierad men ännu inte körbar i labbet.</p></article>
            <article><strong>Engine controls</strong><p><code>@id</code>, <code>@order</code>, <code>@inherit</code> och <code>@cross</code> styr motorn och skickas inte som funktionsargument. Parsern kräver identifierarsträng, ändligt tal respektive uttryckliga policysträngar utan JavaScript-koercion.</p></article>
            <article><strong>Literal syntax</strong><p>Markörer känns endast igen vid logisk radstart utanför fenced code. U+005C före markören gör raden literal; genererad output parsas aldrig om.</p></article>
          ...
          <Callout title="Recovery är editorstruktur, inte tolererad exekvering" icon={<AlertTriangle />} tone="warning">
            Ofullständig syntax ger lokala <code>Recovery</code>-noder med stabil kod och exakt span. Giltiga syskon finns kvar i CST/AST/IR, men varje error-level recovery blockerar modulinitiering, Plan och domänexekvering för hela snapshoten.
          </Callout>
          <Callout title="Varför Lezer — och varför ingen Worker-migrering" icon={<Braces />} tone="info">
            Lezer är JavaScript-native, editororienterat och byggt för syntaxträd under pågående fel. Tree-sitters webbväg hade krävt separat runtime-Wasm, grammar-Wasm och asynkron assetladdning. Parsern genereras därför offline och buntas i samma klassiska <code>/runtime-worker.js</code>; Lezer-trädet förblir intern CST och blir aldrig publikt IR-schema.
          </Callout>
          ...
          <Requirement id="PARSE-001">Samma source snapshot och grammar version MÅSTE deterministiskt ge samma CST, diagnostikkoder och recovery kinds.</Requirement>
          <Requirement id="PARSE-002">Recovery MÅSTE behålla exakt authored span eller en explicit zero-width missing-token-position. Den får aldrig fabricera en körbar opener, close eller stage.</Requirement>
          <Requirement id="PARSE-003">Varje error-level recovery MÅSTE blockera modulinitiering och domänexekvering men partial CST, AST, IR och diagnostik MÅSTE fortfarande kunna levereras till editorn.</Requirement>
        </section>
        ...
        <section className="docs-section spec-section" id="properties-modules">
          ...
          <div className="two-column-copy">
            <div>
              ...
              />
              <Callout title="Alias är språksemantik, inte playgroundsemantik" icon={<AlertTriangle />} tone="warning">
                Draft 0.7 definierar <code>as alias</code>, men <code>textabana.parser/lab-v1</code> kör endast <code>{'>>>>! include "path"'}</code>. Exakta legacyraden <code>{'>>>> include "path"'}</code> stöds utan alias och har företräde framför ett block med namnet <code>include</code>.
              </Callout>
            </div>
            <div>
              <h3>Includes</h3>
        ...
        <section className="docs-section spec-section" id="processing">
          <SectionHeading number="09" layer="Language" title="Processing model — en parser före alla effekter" implementation="partial" />
          <CodeExample
          ...
          <ol className="execution-list spec-execution">
            <li><span>1</span><div><strong>Fånga immutable snapshot</strong><p>Bind documentId, revision, source version och exakta source coordinates.</p></div></li>
            <li><span>2</span><div><strong>Bygg lossless CST</strong><p>Lezer bevarar varje lexem och radslut; fenced code och escapes klassificeras före kontrollsyntax.</p></div></li>
            <li><span>3</span><div><strong>Sänk till AST</strong><p>Bygg blockträdet, typed stages, literalnoder, properties, directives och lokala recovery nodes.</p></div></li>
        ...
        <section className="docs-section spec-section" id="module-manifest">
          ...
          <SpecTable
            ...
            rows={[
              ...
              [<code key="behavior">behavior</code>, "segment-preserving · reordering · expanding · reducing · aggregating · generative", "Mapping och värdeflöde; aldrig purity-signal."],
              ...
              [<code key="effects">effects</code>, "channel:* · artifact:* · network · filesystem · model · external", "Observerbara effekter och cachegräns."],
        ...
        <section className="docs-section spec-section" id="editor-kernel">
          ...
          <p className="lead">Editor Kernel är ett host-neutralt control plane runt parser, compiler och runtime. Den körbara workern använder <code>textabana.editor-kernel/lab-v1</code>. En editor kan analysera en ofullständig revision utan exekvering, välja exakt giltig revision att köra och prenumerera på committade metadataförändringar.</p>
          <div className="architecture-flow" aria-label="Editor Kernel från host till delta">
            ...
            <ArrowDown aria-hidden="true" />
            <div className="architecture-primary"><Workflow aria-hidden="true" /><strong>Editor Kernel</strong><span>open · change · analyze · subscribe · run · cancel</span></div>
            <ArrowDown aria-hidden="true" />
          ...
          <Requirement id="DOCUMENT-003">En host MÅSTE avancera sin protokoll-head från kärnans korrelerade <code>open</code>- eller <code>change</code>-svar. Optimistiskt antagen revision/version får inte användas som grund för nästa change eller run.</Requirement>
          <Requirement id="ANALYZE-001"><code>analyze</code> MÅSTE vara read-only, revisionsbundet och fritt från modulinitiering och stage-effekter. Det FÅR returnera partial IR med recovery även när samma snapshot inte kan köras.</Requirement>
          ...
          <Requirement id="CHANGE-002">Canonical change offsets räknas i Unicode code points. UTF-16-, line/column- eller editor-native-koordinater MÅSTE konverteras av en hostadapter före protokollgränsen; en främmande deklarerad <code>coordinateUnit</code> MÅSTE avvisas före mutation.</Requirement>
          ...
          <SpecTable
            ...
            rows={[
              ...
              ["Parser-/compilerreuse", "Giltiga Lezer-fragment återanvänds efter ChangeSets och en exakt analyserad revision återanvänder sin compiler-snapshot; grafen byggs fortfarande per run.", <StatusBadge key="compile-reuse" tone="implemented">Avgränsad lab-subset</StatusBadge>],
              ["Inkrementell leverans", "Hosten får semantiska deltan och kan välja credit-bunden post-commit-streaming.", <StatusBadge key="delivery" tone="implemented">Implementerat</StatusBadge>],
            ]}
          />
        ...
        <section className="docs-section spec-section" id="conformance">
          ...
          <SpecTable
            ...
            rows={[
              [<code key="lang">language-core/0.4</code>, "Formell parser, lossless CST, typed IR, recovery, block, intervall, property, pipeline, inheritance och cross=error.", <StatusBadge key="c1" tone="partial">Playground subset</StatusBadge>],
          ...
          <div className="conformance-grid">
            <article><CheckCircle2 aria-hidden="true" /><strong>Våg 5 · extern värdsuite</strong><p>CLI:n kör åtta gemensamma protokollfall genom Worker, TypeScript, CodeMirror, Monaco och Python. Rapporten visar 40 utfall och binder suite och kernel till SHA-256. Samma JavaScript-kärna används i alla värdar.</p><a href="/conformance/host-report.json" download>Hämta verifieringsrapporten (JSON)</a></article>
            ...
            <article><CircleDashed aria-hidden="true" /><strong>Återstår för full konformitet</strong><p>Fulla produktionsprofiler för IR/Plan/Result, full JSON Schema, transparent distribuerad cache, cached event replay, multicore stage-exekvering, effectful branch-concurrency, kontinuerlig stage-streaming, persistent dokumenthistorik, strukturell/fuzzy re-anchor, LSP-adapter, preemption, hårda CPU-/minneskvoter, extern side-effect rollback, beständiga artifacts, fulla polyglotta runtimes, Jupyter Messaging och nbformat-roundtrip samt externa observability-profiler.</p></article>
          </div>
          <CodeExample title="Extern verifiering · sprint 5.1" language="bash" status="Körbar värdsuite" code={code("npm run conformance:external", "node cli/textabana.mjs conformance > report.json", "node cli/textabana.mjs digest report.json", "node cli/textabana.mjs registry-check PACKAGE_REGISTRY.json")} />
          <p>CLI:n kan även signera rapporter med en egen Ed25519-nyckel och verifiera dem mot en separat betrodd publik nyckel. De nedladdningsbara rapporterna är osignerade. Befintliga labb-ID:n behålls; sprint 5.2 lägger till separata källbundna SHA-256-identiteter för IR, Plan och committat Result.</p>
          <CodeExample title="Semantiska identiteter · sprint 5.2" language="bash" status="Körbar artefaktprofil" code={code("node cli/textabana.mjs identify examples/document.md > identity.json", "node cli/textabana.mjs verify-identity identity.json", "node cli/textabana.mjs conformance-semantic > semantic-report.json")} />
          <p>Aktivera SHA-256-identiteter i Playground och öppna Conformance → Identiteter för att se artefakterna. Den externa profilen verifierar 28 utfall mot fasta identiteter, inklusive inkrementell parsning och verklig cacheåteranvändning. <a href="/conformance/semantic-report.json" download>Hämta den semantiska profilrapporten (JSON)</a>. Integritetsverifiering bevisar paketets interna konsistens; fullständiga produktionsprofiler, bredare runtimejämförelse, release-signering och registertjänst återstår.</p>
          <CodeExample title="Exekverbart artefaktkontrakt · sprint 5.3" language="bash" status="80 kontraktfall" code={code("node cli/textabana.mjs verify-identity identity.json", "node cli/textabana.mjs conformance-contract > contract-report.json")} />
          <p>Verifieraren kontrollerar nu JSON-struktur, kontrollsummor, identitetskedja, tillgängliga källbytes samt interna referenser mellan IR, plan, events, ankare och source maps. Klicka på Verifiera paket i identitetsvyn, eller ladda ner paketet för samma kontroll i CLI. <a href="/contracts/semantic-bundle-v1.schema.json" download>JSON Schema</a> och <a href="/conformance/contract-report.json" download>kontraktrapporten</a> kan hämtas separat. De 80 frysta fallen omfattar 14 giltiga paket och 66 förväntade avvisningar. En omhashad ändring av render kan fortfarande vara ett giltigt paket: verifieraren kör inte om modulerna och intygar inte exekveringens riktighet.</p>
          <CodeExample title="Oberoende textprofil · sprint 5.4" language="bash" status="70 jämförelsefall" code={code("python3 reference/text_core.py run examples/text-core.md", "node cli/textabana.mjs conformance-text-core > text-core-report.json")} />
          <p>En fristående Python-parser och evaluator kör nu samma avgränsade textprofil som JavaScript-kärnan. Python-versionen behöver varken Node eller projektets JavaScript-kod. De 70 fasta testfallen jämför exakt renderad text, felutfall och commit-status för bland annat nästlade block, pipelines, Unicode och resursgränser. <a href="/conformance/text-core-report.json" download>Hämta rapporten</a>. Anspråket gäller endast textabana.text-core/v1; full språksemantik, moduler, kanaler, cache och identiska IR/Plan/Result-artefakter ingår inte.</p>
          <CodeExample title="Oberoende intervallprofil · sprint 5.5" language="bash" status="80 jämförelsefall" code={code("python3 reference/scoped_text.py run examples/scoped-text.md", "node cli/textabana.mjs conformance-scoped-text > scoped-text-report.json")} />
          <p>Profilen textabana.scoped-text/v1 utökar jämförelsen till öppna intervall, namngivna avslut, numerisk ordning och block som ärver eller stänger av yttre intervall. 80 fasta fall jämför både render och hela ordningen på committade funktionsanrop. Gränsen för antalet anrop kontrolleras före exekvering; felaktiga scope-/blockgränser lämnar inget committat resultat. <a href="/conformance/scoped-text-report.json" download>Hämta intervallrapporten</a>. Godtyckliga moduler, alternativa korsningspolicyer, cache och full produktionskonformitet återstår.</p>
          <CodeExample title="Oberoende kanalprofil · sprint 5.6" language="bash" status="80 jämförelsefall" code={code("python3 reference/channel_core.py run examples/channel-core.md", "node cli/textabana.mjs conformance-channel-core > channel-core-report.json")} />
          <p>Profilen textabana.channel-core/v1 jämför även exakta payloads, global händelseordning och sparade kanalresultat. Tillfälliga händelser behåller sina platsnummer men ingår inte i durable snapshots. 80 fasta fall verifierar bland annat payloadkopiering, obligatoriska tomma kanaler, blockarv och återställning av hela resultatet vid kanal- eller budgetfel. <a href="/conformance/channel-core-report.json" download>Hämta kanalrapporten</a>. Källpositioner och full proveniensekvivalens ingår inte i jämförelsen mellan runtimes.</p>
          <CodeExample title="Oberoende positionsprofil · sprint 5.7" language="bash" status="70 jämförelsefall" code={code("python3 reference/source_map_core.py run examples/source-map-core.md", "node cli/textabana.mjs conformance-source-map-core > source-map-core-report.json")} />
          <p>Profilen textabana.source-map-core/v1 jämför var varje händelse hör hemma i den författade texten: deklarationsrader, källintervall, Unicode-positioner, citat och länkar till ankare och funktionsanrop. 70 fasta fall täcker bland annat tomma block, intervall, återanvända radankare och emoji vid citatgränser. Olika rad-id:n med samma interna ankarnyckel avvisas atomiskt. <a href="/conformance/source-map-core-report.json" download>Hämta positionsrapporten</a>. Full proveniensgraf, editorhistorik och semantisk artefaktekvivalens återstår.</p>
          <CodeExample title="Modulkontrakt · sprint 5.8" language="bash" status="54 verifieringsfall" code={code("node cli/textabana.mjs conformance-module-gate > module-gate-report.json")} />
          <p>Modulgrinden verifierar digest, låsning, grants och entydiga funktionsdeklarationer. Rapportens räknare visar om modulens startkod hann köras: felaktiga deklarationer stoppas före laddning, medan faktiska exportkontrakt kontrolleras efter laddning men före transform. <a href="/conformance/module-gate-report.json" download>Hämta modulrapporten</a>. Detta är en labbgrind runt samma JavaScript-kärna, inte oberoende modulexekvering eller en JavaScript-sandbox.</p>
          <CodeExample title="Oberoende paketkontroll · sprint 5.9" language="bash" status="72 jämförelsefall" code={code("node cli/textabana.mjs conformance-module-admission > module-admission-report.json")} />
          <p>Python och JavaScript jämför nu 72 beslut om manifest, låsning, digests och grants. Python kör ingen modulkod. Godkänd förhandskontroll redovisas separat från senare export- och körningsfel; ett ogiltigt senare paket ska stoppa all startkod. <a href="/conformance/module-admission-report.json" download>Hämta paketjämförelsen</a>. Oberoendet gäller paketkontrollen och omfattar inte modulexekvering eller sandbox-säkerhet.</p>
        ...
        <section className="docs-section spec-section" id="errors">
          ...
          <Requirement id="ERROR-002">För samma malformed construct MÅSTE <code>code</code>, <code>recoveryKind</code> och <code>diagnosticKey</code> vara semantiskt stabila. <code>diagnosticId</code> och source span är snapshotbundna och får flytta sig först genom en explicit ny analys eller re-anchor-transition.</Requirement>
          <Requirement id="ERROR-003">En parsediagnostik MÅSTE bära ett halvöppet <code>sourceSpan</code> och <code>recoveryNodeId</code>. Relaterad opener eller declaration BÖR anges i <code>related</code>.</Requirement>
        ...
        <section className="docs-section spec-section" id="glossary">
          ...
          <SpecTable
            ...
            rows={[
              ...
              [<code key="cst">CST</code>, "Förlustfri konkret syntaxrepresentation från Lezer; bevarar lexem och radslut utan att bestämma domänexekvering."],
              [<code key="ast">AST</code>, "Normaliserat blockträd med typed stages, literalnoder och recovery; intervallhändelser förblir separat modellerade."],
              [<code key="source-span">SourceSpan</code>, "Nollbaserat, halvöppet Unicode-code-point-intervall med 1-baserade line- och 0-baserade column-projektioner."],
              [<code key="recovery">RecoveryNode</code>, "Lokal representation av malformed eller saknad syntax. Den är alltid non-executable men gör partial editorstruktur möjlig."],
