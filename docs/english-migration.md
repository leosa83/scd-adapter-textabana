...
| Surface | Completed through increment 5.14B.3 | Remaining work |
|---|---|---|
...
| Application shell and lab panels | English navigation, controls, metadata, fixture summaries, empty states and accessibility labels. The Parser tab presents English primary and related messages separately from raw artifacts. Conformance reading views now show English stage/requirement explanations, negative-fixture purposes and cancellation limits; overlooked conformance tab labels are corrected. | Adapter messages and embedded module explanations remain sprint 5.14B work. User-authored values and original artifact views retain their original language. |
...
| Runtime and versioned conformance profiles | English diagnostics in channels, editor lifecycle, module admission/loading, scheduler/planning, run policy and cache values. The parser guide is English as of 5.14B.1, with unchanged EBNF. Parser/conformance presentation in 5.14B.2–B.3 leaves raw legacy artifacts unchanged. The six independent profile documents and semantic identity/contract guides were already English and retain their bound bytes. | Review adapter copy and embedded modules against their digests. See the compatibility record for remaining 5.14B work. |
