...
| Surface | Completed through increment 5.14B.4 | Remaining work |
|---|---|---|
...
| Application shell and lab panels | English navigation, controls, metadata, fixture summaries, empty states and accessibility labels. The Parser tab presents English primary and related messages separately from raw artifacts. Conformance reading views show English stage/requirement explanations, negative-fixture purposes and cancellation limits. Adapter diagnostics and the empty capability label are now English. | Embedded module explanations remain sprint 5.14B work. User-authored values and original parser/conformance artifact views retain their original language. |
...
| Runtime and versioned conformance profiles | English diagnostics in channels, editor lifecycle, module admission/loading, scheduler/planning, run policy, cache values and post-commit adapters. The parser guide is English as of 5.14B.1, with unchanged EBNF. Parser/conformance presentation in 5.14B.2–B.3 leaves raw legacy artifacts unchanged. In 5.14B.4, adapter diagnostic IDs change with their English messages; manifest content and lab projection identity rules remain unchanged. The six independent profile documents and semantic identity/contract guides were already English and retain their bound bytes. | Review embedded modules against their source digests. See the compatibility record for remaining 5.14B work and the distinction between lab projection IDs and full-envelope bytes. |
...
Increment 5.14B.4 translates 87 authored adapter string occurrences directly, including the explanatory projection extension note. Five new regressions preserve the observed manifest/output/projection baselines and verify English errors, message-derived diagnostic ID changes, Unicode values and the post-commit boundary through the Worker. All nine CLI reports were rerun: only implementation fingerprints changed in eight reports; the contract report and all frozen expected outcomes remain unchanged. This does not add Arrow/Parquet, notebook-kernel execution, ML lineage or broader standards support.
