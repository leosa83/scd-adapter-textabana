import type { ChangeSetResult, Subscription } from "./responses";
/** Host-neutral runtime data shapes shared by the SDK and playground. */
...
export type EditorKernelRun = {
  ...
  change?: ChangeSetResult | null;
  subscription?: Subscription | null;
  metadataDelta?: EditorMetadataDelta | null;
  deliveries?: Array<{ subscription: Subscription; delta: EditorMetadataDelta }>;
...
/** Committed or rolled-back lab result; this is not the SHA-256 artifact bundle. */
export interface ResultEnvelope {
  schema: "textabana.result/lab-v1";
  resultId: string;
  run: { runId: string; instanceId: string; profile: string; status: "succeeded" | "failed" | "cancelled"; committed: boolean };
  source: { documentId: string; version: string } | null;
  render: { kind: "text"; mediaType: "text/markdown"; data: string };
  channelSnapshots: Record<string, { descriptor: ChannelDescriptor; events: ChannelEvent[] }>;
  anchors: RuntimeAnchor[];
  sourceMaps: RuntimeSourceMap[];
  artifacts: never[];
  provenance: {
    entities: Array<{
      entityId: string;
      entityType: "cached-stage-output" | "cache-observation";
      planNodeRef: string | null;
      outputDigest: string;
    ...
    activities: Array<{
      activityId: string; runRef: string; planNodeRef: string | null; stageId: string; invocationId: string;
      function: string; orderKey: [number, number, number];
      activityType: "cache-materialization" | "transform-invocation";
      executionMode: string; functionInvoked: boolean; cacheEntryRef?: string; evidenceRefs?: string[];
    }>;
    agents: never[];
  };
  diagnostics: RuntimeDiagnostic[];
  hashes: { ir: string | null; environment: string };
  extensions: { "textabana.playground": { canonical: false; note: string; duration: number; error?: string } };
}

export interface RuntimeCapabilities {
  schema: "textabana.capabilities/lab-v1";
  optionalProfiles: Array<{ profile: string; activation: string; available: boolean; scope: string; fullRuntimeConformance: boolean }>;
  profiles: Record<string, AdapterSupport>;
  adapters: Array<Pick<AdapterManifest, "adapterId" | "version" | "profile" | "support" | "manifestDigest">>;
